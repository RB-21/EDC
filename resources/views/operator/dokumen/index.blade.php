...
@section('content')
    ...
    <form id="tampilForm" method="post"  enctype="multipart/form-data" action="{{ route('operator.dokumen.tampil') }}" target="result">
        @csrf
...
<div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
        ...
        <form action="{{ route('operator.dokumen.simpan') }}" method="POST" enctype="multipart/form-data">
            @csrf
...
<div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
        ...
        <form action="{{ route('operator.dokumen.edit') }}" method="POST" enctype="multipart/form-data">
            @csrf
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            let tableUser = $('#table-user').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('operator.dokumen.getDataDokumen') }}",
                    method: 'POST',
                    data: {
                        _token: "{{ csrf_token() }}"
                    }
            ...
            let tableDokumenHistory = $('#tableDokumenModal').DataTable({
                ...
                deferLoading: 0,
                ajax: {
                    url: "{{ route('operator.dokumen.getDokumenHistory') }}",
                    method: 'POST',
                    data: function(d){
                        d._token =  "{{ csrf_token() }}"
            ...
            let tablePerubahanDokumenHistory = $('#tablePerubahanDokumenModal').DataTable({
                ...
                deferLoading: 0,
                ajax: {
                    url: "{{ route('operator.dokumen.getPerubahanDokumenHistory') }}",
                    method: 'POST',
                    data: function(d){
                        d._token =  "{{ csrf_token() }}"
            ...
            $('input[name="nomor"]').keyup(function() {
                ...
                $.ajax({
                    url: '{{ route('operator.dokumen.check-nomor') }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        nomor: value
            ...
            $(document).on('keyup', '#tambahModal .perubahan-container .bs-searchbox input[type=search]', function(){
                let input = $(this).val()
                $.ajax({
                    url: "{{ route('operator.dokumen.getDokumenPerubahan') }}",
                    method: 'POST',
                    data: {
                        _token: "{{ csrf_token() }}",
                        input: input
