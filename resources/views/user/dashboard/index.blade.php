    ...
    <section class="section">
        ...
        <div class="row">
            ...
            <div class="col-lg-3 col-md-6 col-sm-6 col-12">
                <a href="{{ route('user.dokumen.index_by_jenis', [$item->kode, $item->bagian->kode_bagian]) }}"
        ...
        @else
            <div class="col-lg-3 col-md-6 col-sm-6 col-12">
                <a href="{{ route('user.dokumen.index_by_jenis', [$item->kode]) }}" style="text-decoration:none">
                    <div class="card card-statistic-1">
                        <div class="card-icon bg-{{ $item->bg_color }}">
                            <i class="{{ $item->fa_icon }}"></i>
                        </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table id="table-{{ $item->kode }}" class="table table-sm table-bordered align-items-center w-100" id="table-user">
                                    <thead>
                                        <tr>
                                            <th>No</th>
                                            <th>Nomor</th>
...
<script type="text/javascript" src="https://cdn.datatables.net/v/bs4/dt-1.13.1/datatables.min.js"></script>
    <script>
        $(document).ready(function(){
            @foreach ($count_per_jenis_file_category as $item)
                {{-- beri variable dengan nama aksi ditambah dengan item kode => aksi --}}
                let aksi_{{ $item->kode }} = {!! $item->jenis_aksi !!}
                console.log(aksi_{{ $item->kode }})
            $('#table-{{ $item->kode }}').DataTable({
                dom: 'rt',
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('user.getDataDokumenByJenis') }}",
                    method: 'POST',
                    data: {
                        _token: "{{ csrf_token() }}",
