            ...
            @foreach ($count_per_jenis_file_category as $item)
                @if ($item->has_sub)
                    <div class="col-lg-3 col-md-6 col-sm-6 col-12">
                        <a href="{{ route('admin.dokumen.index_by_jenis', [$item->kode, $item->bagian->kode_bagian]) }}"
                ...
                @else
                    <div class="col-lg-3 col-md-6 col-sm-6 col-12">
                        <a href="{{ route('admin.dokumen.index_by_jenis', [$item->kode]) }}" style="text-decoration:none">
                            <div class="card card-statistic-1">
                                <div class="card-icon bg-{{ $item->bg_color }}">
                                    <i class="{{ $item->fa_icon }}"></i>
                                </div>
        ...
        $(document).ready(function() {
            @foreach ($count_per_jenis_file_category as $item)
                $('#table-{{ $item->kode }}').DataTable({
                    ...
                    serverSide: true,
                    ajax: {
                        url: "{{ route('admin.getDataDokumenByJenis') }}",
                        method: 'POST',
                        data: {
                            _token: "{{ csrf_token() }}",
