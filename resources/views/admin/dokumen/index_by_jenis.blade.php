...
@section('content')
    ...
    <form id="tampilForm" method="post"  enctype="multipart/form-data" action="{{ route('admin.dokumen.tampil') }}" target="result">
        @csrf
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            let tableUser = $('#table-user').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('admin.dokumen.getDataDokumenByJenis') }}",
                    method: 'POST',
                    data: {
                        _token: "{{ csrf_token() }}",
